...
import { transactions as transactionsIndex } from '@/actions/App/Http/Controllers/DashboardController';
...
import { pdf as belegPdf } from '@/routes/belege';
import type {
    FlashProps,
    TransactionAssociationBelegRow,
...
function BelegAssociationCard({
    beleg,
    onAssociate,
    onDetach,
}: {
    beleg: TransactionAssociationBelegRow;
    onAssociate: (belegId: number) => void;
    onDetach: (belegId: number) => void;
}) {
    return (
                ...
                <div className="space-y-3">
                    <div className="flex flex-wrap items-center gap-2">
                        <Badge
                            className={cn(
                                'rounded-full',
                                associationBadgeClassName(beleg.is_associated),
                            )}
                            variant="secondary"
                        >
                            {beleg.is_associated ? 'zugeordnet' : 'offen'}
                        </Badge>
                        <Badge className="rounded-full" variant="outline">
                            {dayOffsetLabel(beleg.days_offset)}
                        </Badge>
                        {beleg.document_type && (
                            <Badge className="rounded-full" variant="outline">
                                {beleg.document_type}
                    ...
                    <div className="space-y-1">
                        <h2 className="text-lg font-semibold text-slate-950">
                            {beleg.issuer_name ?? 'Unbekannter Beleg'}
                        </h2>
                    ...
                    <dl className="grid gap-3 text-sm text-slate-600 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <dd className="mt-1 font-medium text-slate-900">
                                {formatCurrency(beleg.gross_amount_cents)}
                            </dd>
                        </div>
                        <div>
                            <dt className="text-xs tracking-[0.18em] text-slate-500 uppercase">
                ...
                <div className="flex shrink-0 flex-wrap items-center gap-2">
                    ...
                    <Button
                        ...
                        onClick={() =>
                            beleg.is_associated
                                ? onDetach(beleg.id)
                                : onAssociate(beleg.id)
                        }
                        variant={beleg.is_associated ? 'outline' : 'default'}
                    >
                        {beleg.is_associated ? (
                            <>
                                Lösen
...
export default function TransactionShow({
    ...
    dateWindow,
    nearbyBelege,
    associatedBelegeOutsideWindow,
}: TransactionDetailPageProps) {
    const { flash } = usePage<{ flash: FlashProps }>().props;

    const backHref = transactionsIndex.url({
        query: {
    ...
    const associateBeleg = (belegId: number) => {
        router.visit(TransactionBelegAssociationController.store(transaction.id), {
            data: {
                imported_beleg_id: belegId,
            },
            method: 'post',
            preserveScroll: true,
        });
    ...
    const detachBeleg = (belegId: number) => {
        router.visit(
    ...
    return (
        <>
            <Head title={`Transaktion ${transaction.id}`} />
            ...
            <AppShell>
                <div className="space-y-6 pb-10">
                            ...
                            <CardContent className="space-y-8 px-6 py-7 sm:px-8 sm:py-8">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <p className="max-w-3xl text-lg leading-8 text-slate-200">
                                            Vorgeschlagene Belege für{' '}
                                            {formatDate(transaction.booking_date)}{' '}
                                            im Fenster von{' '}
                                            {dateWindow.days} Tagen vor und nach
                                            der Buchung.
                                        </p>
                                    </div>
                                </div>
                                ...
                                <dl className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                                    <div className="rounded-[1.4rem] border border-white/10 bg-white/10 p-4">
                                        <dt className="text-xs tracking-[0.24em] text-slate-300 uppercase">
                                            Betrag
                                        </dt>
                        ...
                        <CardContent className="space-y-4">
                            {nearbyBelege.length === 0 ? (
                                <div className="rounded-[1.6rem] border border-dashed border-slate-200 bg-slate-50/70 px-5 py-10 text-center text-sm text-slate-500">
                                    Im Zeitraum wurden keine Belege gefunden.
                                </div>
                            ) : (
                                nearbyBelege.map((beleg) => (
                                    <BelegAssociationCard
                                        beleg={beleg}
                                        key={beleg.id}
                                        onAssociate={associateBeleg}
                                        onDetach={detachBeleg}
                                    />
                                ))
                            )}
                        </CardContent>
                    ...
                    {associatedBelegeOutsideWindow.length > 0 && (
                        <Card className="overflow-hidden rounded-[2rem] border-white/80 bg-white/90 shadow-[0_20px_55px_rgba(15,23,42,0.06)]">
                            <CardHeader className="pb-4">
                                <CardTitle
                                    className="text-2xl text-slate-950"
                            ...
                            <CardContent className="space-y-4">
                                {associatedBelegeOutsideWindow.map((beleg) => (
                                    <BelegAssociationCard
                                        beleg={beleg}
                                        key={beleg.id}
                                        onAssociate={associateBeleg}
